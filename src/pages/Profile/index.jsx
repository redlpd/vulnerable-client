...
const Profile = () => {
  ...
  const { data: creditsHistoryData, loading: creditsHistoryLoading } = useQuery(
    ListCreditsHistoryQuery
  )
  const { data: userCreditsData, loading: userCreditsLoading } =
    useQuery(GetUserCreditsQuery)
  const [addCredits, { loading: addCreditsLoading }] =
    useMutation(AddCreditsMutation)
  const [sendCredits, { loading: sendCreditsLoading }] =
    useMutation(SendCreditsMutation)
  ...
  const mutation =
    modalData.type === 'add' ? handleAddCredits : handleSendCredits

  if (loading || creditsHistoryLoading || userCreditsLoading)
          ...
          <div className='w-full'>
            <div className='flex justify-between items-center mb-5'>
              <label
                ...
                style={{ color: '#9e9e9e' }}
              >
                <span className={styles.bullet} />A C C O U N T
              </label>
              ...
              <div className='flex items-center'>
                ...
                <button
                  ...
                  onClick={() =>
                    setModalData({ showModal: true, type: 'send' })
                  }
                >
                  SEND
                </button>
            ...
            <label
              className='text-base font-medium flex items-center mt-10 mb-5'
              style={{ color: '#9e9e9e' }}
            >
              <span className={styles.bullet} />C R E D I T S &nbsp;&nbsp; H I S
              T O R Y
            </label>
            <div className={styles.infoContainer}>
              {creditsHistory.length ? (
                  ...
                  {creditsHistory.map((credit) => (
                    <div className='flex justify-between my-2' key={credit.id}>
                      ...
                      <p
                        ...
                        style={{ color: credit.amount < 0 ? 'red' : '#11823b' }}
                      >
                        {`PHP ${amountFormatter(credit.amount)}`}
                      </p>
                    </div>
                  ))}
                </>
                ...
                <div className='text-center'>
                  ...
                  <button
                    type='button'
                    className={`${styles.actionBtn} ${styles.green} mt-4`}
                    onClick={() =>
                      setModalData({ showModal: true, type: 'add' })
                    }
                  >
                    ADD NOW
                  </button>

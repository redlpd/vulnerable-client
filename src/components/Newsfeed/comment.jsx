  ...
  const handleCreateComment = () => {
    createComment({
      ...
      update(cache, { data: { createComment } }) {
        ...
        // const comments = [...newListPosts[index].comments]
        newListPosts[index].comments.unshift(createComment)
        // newListPosts[index] = [...comments]

        console.log(newListPosts, newListPosts[index].comments, comments)
        // cache.writeQuery({
        ...
        {comments.map((comment) => (
          <div className={styles.commentContainer} key={comment.id}>
            <div className='flex items-center'>
              ...
              <span className='font-medium text-sm' style={{ color: 'rgb(36 36 36)' }}>{comment.user.fullName}</span>
            </div>
